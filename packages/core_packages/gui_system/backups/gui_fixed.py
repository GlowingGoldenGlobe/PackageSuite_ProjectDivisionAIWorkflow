...
from gui.gui_implementation import TimeLimit, ModelVersionDisplay

# Define the apply_gui_enhancements function directly
def apply_gui_enhancements(app):
    """Apply GUI enhancements to the application
    This is a compatibility function that might be needed by older code
    """
    try:
        # Try to import from original location for backward compatibility
        from agent_mode_gui_implementation import apply_gui_enhancements as original_apply
        original_apply(app)
    except (ImportError, AttributeError):
        # If not available, do nothing
        print("GUI enhancement function not found in original location")
        pass

# Main application class
class AgentModeStarter:
    ...
    def start_new_session(self):
        ...
        try:
            ...
            from gui.gui_implementation import ModelVerification
            version = self.selected_version.get()
    ...
    def setup_main_tab(self):
        ...
        # Import from the gui folder implementation file
        from gui.gui_implementation import AgentModeSelector
        self.execution_mode, self.api_key, self.execution_style, self.cpu_limit, self.ram_limit, self.confirm_tasks = AgentModeSelector.setup_execution_mode_ui(main_frame, 0)
        ...
        main_frame.grid_columnconfigure(0, weight=1)  # Makes the frame expand horizontally
        ...
        # Use grid for radio buttons instead of pack
        ttk.Radiobutton(mode_frame, text="Refined Model", 
                       variable=self.development_mode, value="refined_model").grid(row=0, column=0, sticky="w")
        ttk.Radiobutton(mode_frame, text="Versioned Model", 
                       variable=self.development_mode, value="versioned_model").grid(row=1, column=0, sticky="w")
        ttk.Radiobutton(mode_frame, text="Testing Only", 
                       variable=self.development_mode, value="testing_only").grid(row=2, column=0, sticky="w")
        ...
        self.version_dropdown.grid(row=0, column=0, sticky="w")
        ...
        self.part_dropdown.grid(row=0, column=0, sticky="w")
        ...
        start_frame.grid(row=2, column=0, sticky="ew", padx=10, pady=10)
        
        # Main start buttons
        ttk.Button(start_frame, text="Start New Development Session", 
                  command=self.start_new_session).grid(row=0, column=0, sticky="ew", padx=5, pady=5)
        ...
        from gui.gui_implementation import ProcessControl
        control_frame = ttk.Frame(start_frame)
        control_frame.pack(fill=tk.X, pady=5)
          # Create start and stop buttons
        self.start_button, self.stop_button, self.pause_button, self.task_status_label = ProcessControl.setup_process_controls(
            start_frame, 
    ...
    def mark_task_complete(self, list_widget):
        ...
        if selection:
            ...
            for task in tasks:
                if task["title"] == clean_text:
                    task_id = task["id"]
                    break
                    
            if task_id:
                # Use TaskCompletion class to mark the task complete
                from gui.gui_implementation import TaskCompletion
                TaskCompletion.mark_task_completed(
                    self.task_manager, 
                    task_id, 
                    list_widget, 
                ...
                # Legacy approach for tasks not in task manager
    ...
    def setup_objectives_tab(self):
        ...
        try:
            # Check if the project objectives module has the OBJECTIVES attribute
            objectives = getattr(project_objectives, 'OBJECTIVES', None)
            
            # If not found, use default objectives
